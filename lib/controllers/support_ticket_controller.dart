...
class SupportTicketController extends GetxController {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == 'success') {
        ...
        if (fetchedData.isNotEmpty) {
          ticketList.addAll(t.TicketListModel.fromJson(data).data!.data!);
          if (isLoadMoreRunning == false) {
            _isLoading = false;
          }
          if (kDebugMode) {
  ...
  List<viewTicket.Data> viewTicketList = [];
  Future getTicketView(
    ...
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      if (data['status'] == 'success') {
        viewTicketList.add(viewTicket.ViewTicketModel.fromJson(data).data!);
        update();
      } else {
        ApiStatus.checkStatus(data['status'], data['data']);
      }
