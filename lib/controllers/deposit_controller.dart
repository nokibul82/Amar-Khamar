...
class DepositController extends GetxController {
  ...
  Future getSelectedCurrencyData(value) async {
    ...
    // if the payment from project or plan invest
    await ProjectController.to.calculateCheckoutAmount();

    // if the payment from ecommerce section
    await ProductManageController.to.calculateCheckoutAmount();
      ...
      if (amountValue.isNotEmpty && selectedCurrency != null) {
        totalChargedAmount =
            (double.parse(amountValue.toString()) + double.parse(charge))
                .toStringAsFixed(2);
        totalPayableAmountInBaseCurrency =
  ...
  Future manualPayment(
    ...
    if (response.statusCode == 200) {
      if (data['status'] == 'success') {
        ...
        if (ProductManageController.to.selectedPaymentMethod == "checkout" &&
  ...
  String trxId = "";
  Future addFundRequest(
      {required Map<String, String> fields,
      required BuildContext context}) async {
    isLoadingPaymentSheet = true;
    update();
    http.Response response = await DepositRepo.addFundRequest(fields: fields);
    isLoadingPaymentSheet = false;
    update();
    var data = jsonDecode(response.body);
    if (response.statusCode == 200) {
  ...
  Future webviewPayment({required String trxId}) async {
    _isLoading = true;
    update();
    http.Response response = await DepositRepo.webviewPayment(trxId: trxId);
    _isLoading = false;
    update();
  ...
  Future onPaymentDone({required Map<String, String> fields}) async {
    isLoadingPaymentSheet = true;
    update();
    http.Response response = await DepositRepo.onPaymentDone(trxId: this.trxId);
    isLoadingPaymentSheet = false;
    update();
    var data = jsonDecode(response.body);
    if (response.statusCode == 200) {
