  ...
  Future getTwoFa() async {
    ...
    if (response.statusCode == 200) {
      if (data['status'] == 'success') {
        print("================${data['data']['towFactorEnable']}");
        isTwoFactorEnabled = data['data']['towFactorEnable'] ?? false;
    ...
    if (response.statusCode == 200) {
      if (data['status'] == 'success') {
        ...
        if (data['data'] != null && data['data'] is List) {
          List<Map<String, dynamic>> kForm = List.from(data['data']);

          for (int i = 0; i < kForm.length; i += 1) {
            categoryNameList.add(CategoryNameModel(
              status: kForm[i]['status'] ?? "",
              id: kForm[i]['id'],
              slug: kForm[i]['slug'] ?? "",
